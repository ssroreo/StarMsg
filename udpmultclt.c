...
int main(int argc, char* argv[]) {
    ...
    if(pid == 0) { //子进程,先忽略退出处理防止成为僵尸进程
        ...
        while(fgets(msg.text, _INT_TEXT, stdin)){
            ...
            msg.type = '2';
            IF_CHECK(sendto(sd, &msg, sizeof msg, 0, (struct sockaddr*)&addr, alen));
			printf("\33[1A\033[1;4;34m%s\033[0m : %s", msg.name, msg.text);
        }
        // 处理结算操作,并杀死父进程
        close(sd);
    ...
    for(;;){
        ...
        switch(msg.type){
        case '1':printf("\033[1;33m%s logined!\033[0m\n", msg.name);break;
        case '2':printf("\033[1;4;32m%s\033[0m : %s", msg.name, msg.text);break;
        case '3':printf("%s quited!\n", msg.name);break;
